...
export default function PayrollMonthDrawer({
  ...
  return (
    <ResponsiveDrawer open={open} onClose={onClose} title={`Xử lý lương tháng ${period}`} widthClassName="max-w-3xl">
      <div className="space-y-4">
        <p className="text-sm text-[#64748b]">{lineCount} người đã có dòng lương trong tháng này.</p>

        <PayrollRunActions runId={runId} status={status} checklistReady={checklist?.isReady ?? true} />

        {checklist ? (
          <Section title="Checklist chốt tháng" hint={checklist.isReady ? "Đạt" : "Chưa đạt"} defaultOpen={!checklist.isReady}>
