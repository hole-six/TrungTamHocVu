...
export async function getBatchInvoiceViewData(periodId: string) {
  ...
  const charges = chargesWithRemaining.map((c) => {
    ...
    return {
      ...
      mainTuitionAmount: c.mainTuitionAmount,
      paidCatchupAmount: c.paidCatchupAmount,
      transferCreditAmount: c.transferCreditAmount,
      transferRemainderAmount: c.transferRemainderAmount,
