...
async function ensurePaymentForCharge(args: {
  ...
  const allocationAmount = args.allocationAmount ?? args.amount;
  if (allocationAmount > 0) {
    await prisma.paymentAllocation.upsert({
      ...
      create: { paymentId: payment.id, chargeId: args.chargeId, amount: allocationAmount },
    });
  } else {
    await prisma.paymentAllocation.deleteMany({
      where: { paymentId: payment.id, chargeId: args.chargeId },
    });
  }
...
async function main() {
  ...
  if (overpayCharge) {
    const overpayAmount = Math.round(overpayCharge.totalAmount * 0.1);
    ...
    const existingCredit = await prisma.creditBalance.findFirst({
      where: { paymentId: paid.id, studentId: overpayCharge.studentId, reason: `${TAG} overpay` },
    });
    if (existingCredit) {
      await prisma.creditBalance.update({
        where: { id: existingCredit.id },
        data: { amount: overpayAmount },
      });
    } else {
